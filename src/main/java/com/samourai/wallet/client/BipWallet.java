...
public class BipWallet {
  ...
  public BipWallet(
    ...
    this.hdWallet = new HD_Wallet(addressType.getPurpose(), bip44w);
    this.indexHandler = indexHandler;
    this.indexChangeHandler = indexChangeHandler;
  }

  public String getPub(AddressType addressType) {
    HD_Account hdAccount = hdWallet.getAccountAt(account.getAccountIndex());
    switch (addressType) {
      case LEGACY:
  ...
  public HD_Address getAddressAt(int chainIndex, int addressIndex) {
    return hdWallet.getAddressAt(account.getAccountIndex(), chainIndex, addressIndex);
  }
