...
public class WhirlpoolWalletTx0Test extends Tx0ServiceV1Test {
  ...
  public void tx0Cascade_test0() throws Exception {
    ...
    // run
    List<Tx0> tx0s =
        whirlpoolWallet.tx0Cascade(Arrays.asList(spendFromUtxo), tx0Config, pool);
  ...
  public void tx0Cascade_test2() throws Exception {
    ...
    int totalNbPremix = tx0_pool5.getNbPremix()
    ...
    Assertions.assertTrue(
            utxosContains(
                    tx0_pool5.getSpendFroms(), spendFromUtxo.tx_hash, spendFromUtxo.tx_output_n));

    // tx0_pool05 spends from tx0_pool5 outputs
    Assertions.assertEquals(
            tx0_pool5.getChangeOutputs().size(), tx0_pool05.getSpendFroms().size());
    for (TransactionOutput txOut : tx0_pool5.getChangeOutputs()) {
      String txid = tx0_pool5.getTx().getHashAsString();
      Assertions.assertTrue(
              utxosContains(tx0_pool05.getSpendFroms(), txid, txOut.getIndex()));
    }
